...
// Завдання 2 (advanced) Створіть генератор, який буде регулювати розміри шрифту для вашого сайту. (Можна допрацювати, щоб реально змінював шрифт, але це не є обов'язковою умовою)

export function* newFontGenerator(init){
    let fz = init
    for (let i= 1; i < Infinity; i++) {
       const value = yield fz;
        if(value === "up"){
            fz += 2
        }
        if (value === "down"){
